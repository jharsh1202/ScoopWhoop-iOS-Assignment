...
var showRelatedVideo = [ShowRelatedVideos]()
...
// MARK: - ShowDetails
struct ShowRelatedVideosResponse: Codable {
    let status: Int
    let showDetails: ShowDetails
    let nextOffset: Int
    let data: [ShowRelatedVideoResponse]
    let err: String
...
// MARK: - Datum
struct ShowRelatedVideoResponse: Codable {
    let castCrew: [JSONAny]
    let shHeading, title: String
    let newTags: [NewTag]
    ...
    let nsfw: Int
    let featureImg, videoThumbnail16X9: String
    let duration: String
    let premium: Bool
